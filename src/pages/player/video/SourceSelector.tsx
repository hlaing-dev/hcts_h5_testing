  ...
  return (
    <div className="bg-white dark:bg-[#161619] p-4 mb-4">
      ...
      <div className="flex justify-between items-center mb-2">
        <h4 className="text-black dark:text-white text-[18px] font-bold">
          选集
        </h4>
      ...
      {/* Source Selector Area */}
      {movieDetail && movieDetail.play_from && (
        <div
          className="p-4 flex justify-between items-center rounded-lg shadow-sm mt-5"
          style={{
            background: "linear-gradient(100deg, #FEE4B3 0%, #FFD993 100%)",
          }}
        >
          <div className="text-channel leading-tight">
            ...
            <span className="text-sm">
              <span className="font-bold">
                {movieDetail.play_from.filter(
                  (x) => x.code === selectedEpisode?.from_code
                )[0]?.name || "未选择来源"}
              </span>{" "}
              <span className="text-channelSecondary">
                {movieDetail?.play_from[selectedSource]?.total || 0} 个视频
              </span>
            </span>
          </div>
